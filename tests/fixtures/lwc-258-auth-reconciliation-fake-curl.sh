...
if [[ "$url" == *"/actions/runs/$ATTEMPT_RUN_ID/artifacts?"* ]]; then
  ...
  case "$scenario" in
    ...
    duplicate) printf '%s' '{"envs":[{"key":"NEXT_PUBLIC_AUTH_URL","value":"https://auth-dev.rayer.idv.tw","type":"sensitive","target":["preview"],"gitBranch":"develop"},{"key":"NEXT_PUBLIC_AUTH_URL","value":"https://auth-dev.rayer.idv.tw","type":"sensitive","target":["preview"],"gitBranch":"develop"}]}' ;;
    ...
    *) printf '%s' '{"envs":[{"key":"NEXT_PUBLIC_AUTH_URL","value":"https://auth-dev.rayer.idv.tw","type":"sensitive","target":["preview"],"gitBranch":"develop"}]}' ;;
  esac
else
  exit 1
fi
