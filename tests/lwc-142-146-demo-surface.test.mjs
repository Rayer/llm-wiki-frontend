...
test('main-path chrome strings are i18n-backed in zh-TW', async () => {
  const [homeClient, pipelineClient, rawClient, listClient, conceptDetailPage, sourceDetailPage, commandPalette, zhTW] =
    await Promise.all([
      readFile(new URL('../src/components/HomeClient.tsx', import.meta.url), 'utf8'),
      ...
      readFile(new URL('../src/app/concepts/[slug]/page.tsx', import.meta.url), 'utf8'),
      readFile(new URL('../src/app/sources/[slug]/page.tsx', import.meta.url), 'utf8'),
  ...
  assert.match(conceptDetailPage, /label=\{t\('Entry\.singular'\)\}/);
  assert.match(sourceDetailPage, /label=\{t\('Source\.singular'\)\}/);
  ...
  assert.equal(zhTW.Pipeline.addContentDescription, '上傳 markdown 檔案或擷取網址，以建立知識資料來源。');
  assert.equal(zhTW.Pipeline.uploadHint, '可同時上傳多個檔案到 raw/ 目錄（最多 3 檔同時上傳）。');
  ...
  assert.equal(zhTW.Pipeline.pipelineStages, 'ingest（分析原始筆記）→ compile（綜合 wiki 條目）→ lint（品質檢查）→ publish（自動核准）。');
});
